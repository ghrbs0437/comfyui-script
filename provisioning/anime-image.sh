#!/bin/bash
# Anime image stack (Illustrious-XL + IP-Adapter) — vast.ai ComfyUI image compatible.
# real-image.sh와 같은 방식: 실제 ComfyUI 설치를 자동 탐지해 그 안에 설치한다.
#
# 폴더 매핑 (구 ai-dock storage → 실제 ComfyUI models 폴더명):
#   ckpt→checkpoints, unet→diffusion_models, lora→loras, esrgan→upscale_models
#
# Overrides:
...
IPADAPTER_MODELS=(
    # IP-Adapter (SDXL, plus, ViT-H)
    "https://huggingface.co/h94/IP-Adapter/resolve/main/sdxl_models/ip-adapter-plus_sdxl_vit-h.safetensors"
)

CLIP_VISION_MODELS=(
    # CLIP-Vision (ViT-H, IP-Adapter 입력 인코더)
    # 주의: 원본 파일명이 model.safetensors라 그대로 받으면 구분이 어렵다.
    # 받은 뒤 CLIP-ViT-H-14-laion2B-s32B-b79K.safetensors로 rename한다 (아래 RENAME_ITEMS).
...
# "카테고리|원본이름|바꿀이름"
RENAME_ITEMS=(
    "clip_vision|model.safetensors|CLIP-ViT-H-14-laion2B-s32B-b79K.safetensors"
)
